import json
import unittest
from pathlib import Path


EXTENSION_DIR = Path(__file__).resolve().parents[1] / "extensions" / "linkvault-companion"


class ExtensionAssetsTest(unittest.TestCase):
    def test_manifest_and_core_assets_exist(self):
        expected_files = [
            "manifest.json",
            "shared.js",
            "popup.html",
            "popup.js",
            "options.html",
            "options.js",
            "styles.css",
            "README.md",
        ]
        for filename in expected_files:
            self.assertTrue((EXTENSION_DIR / filename).exists(), filename)

        manifest = json.loads((EXTENSION_DIR / "manifest.json").read_text(encoding="utf-8"))

        self.assertEqual(manifest["manifest_version"], 3)
        self.assertIn("bookmarks", manifest["permissions"])
        self.assertIn("storage", manifest["permissions"])
        self.assertIn("http://*/*", manifest["host_permissions"])
        self.assertIn("http://*/*", manifest["optional_host_permissions"])
        self.assertEqual(
            manifest["browser_specific_settings"]["gecko"]["id"],
            "linkvault-companion@nanja-at-web.local",
        )
        extension_csp = manifest["content_security_policy"]["extension_pages"]
        self.assertIn("connect-src http: https:", extension_csp)
        self.assertEqual(manifest["action"]["default_popup"], "popup.html")

    def test_extension_uses_linkvault_api_and_bookmarks_api(self):
        shared = (EXTENSION_DIR / "shared.js").read_text(encoding="utf-8")
        popup = (EXTENSION_DIR / "popup.js").read_text(encoding="utf-8")
        popup_html = (EXTENSION_DIR / "popup.html").read_text(encoding="utf-8")
        options = (EXTENSION_DIR / "options.js").read_text(encoding="utf-8")

        self.assertIn("/api/import/browser-bookmarks/preview", shared)
        self.assertIn("/api/import/browser-bookmarks", shared)
        self.assertIn("/api/export/browser-bookmarks", shared)
        self.assertIn("/api/export/browser-bookmarks/restore-preview", shared)
        self.assertIn("/api/export/browser-bookmarks/restore-complete", shared)
        self.assertIn("/api/conflicts/", shared)
        self.assertIn("/api/bookmarks", shared)
        self.assertIn("bookmarks.getTree", shared)
        self.assertIn("bookmarks.getSubTree", shared)
        self.assertIn("bookmarks.create", shared)
        self.assertIn("bookmarks.update", shared)
        self.assertIn("bookmarks.move", shared)
        self.assertIn("browserBookmarkFolders", shared)
        self.assertIn("browserExistingItems", shared)
        self.assertIn("browserExistingFolders", shared)
        self.assertIn("browserBookmarksToItems", shared)
        self.assertIn("normalizeBookmarkFilters", shared)
        self.assertIn("bookmarkMatchesFilters", shared)
        self.assertIn("importLinkVaultIntoBrowser", shared)
        self.assertIn("previewLinkVaultBrowserImport", shared)
        self.assertIn("completeLinkVaultBrowserRestore", shared)
        self.assertIn("setConflictDecision", shared)
        self.assertIn("duplicateAction", shared)
        self.assertIn("skip_existing", shared)
        self.assertIn("update_existing", shared)
        self.assertIn("merge_existing", shared)
        self.assertIn("requestLinkVaultHostPermission", shared)
        self.assertIn("hostPermissionPattern", shared)
        self.assertIn("isRegularWebUrl", shared)
        self.assertIn("discoverLinkVaultServers", shared)
        self.assertIn(".well-known/linkvault", shared)
        self.assertIn("mapWithConcurrency", shared)
        self.assertIn("saveCurrentTab", popup)
        self.assertIn("selectedBookmarkSource", popup)
        self.assertIn("selectedBrowserRestoreOptions", popup)
        self.assertIn("renderPreviewDetails", popup)
        self.assertIn("renderBrowserRestorePreview", popup)
        self.assertIn("importLinkVaultIntoBrowser", popup)
        self.assertIn("previewLinkVaultBrowserImport", popup)
        self.assertIn("lastBrowserRestoreDecisions", popup)
        self.assertIn("Conflict decision", popup)
        self.assertIn("Folder decision", popup)
        self.assertIn("clear-bookmark-filters", popup)
        self.assertIn("import-into-browser", popup_html)
        self.assertIn("preview-browser-restore", popup_html)
        self.assertIn("browser-restore-target-mode", popup_html)
        self.assertIn("browser-restore-target-folder", popup_html)
        self.assertIn("browser-restore-duplicates", popup_html)
        self.assertIn("linkvault-export-query", popup_html)
        self.assertIn("linkvault-export-collection", popup_html)
        self.assertIn("bookmark-filter-query", popup_html)
        self.assertIn("bookmark-filter-domain", popup_html)
        self.assertIn("bookmark-filter-added-from", popup_html)
        self.assertIn("bookmark-filter-added-to", popup_html)
        self.assertIn("PREVIEW_DETAIL_LIMIT", popup)
        self.assertIn("find-linkvault", options)
        self.assertIn("scan-subnet", options)
        self.assertIn("subnetPrefixFromUrl", options)
        self.assertIn("testLinkVaultConnection", options)


if __name__ == "__main__":
    unittest.main()
